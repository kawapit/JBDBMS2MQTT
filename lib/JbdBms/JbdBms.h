...
// debug levels:
#define DBG
#define DBG_BLE1	0b00000010		// debug ble i/o
#define DBG_BLE2	0b00000100		// include raw packages
#define DBG_BLE3	0b00001000		// control messages (mosfet,balance)
#define DBG_MQTT	0b00010000		// debug mqtt messages
#define DBG_WEB		0b00100000		// debug web socket messages
...
#define BALANCEINFO	6		// read balance information
#define BALANCE		0x2D	// enable/disable cell balancing
#define MOSFET		0xE1	// set charge/discharge
...
typedef struct {
	char Hardware[25];				// bms hardware version (BS202308-01-104)
									//						(BSJBD202210-24-0007)
...
//	uint32_t SoC_Wh;				// Wh
	...
	uint8_t MosFet, charge, discharge;
...
//	static BLEAdvertisedDevice *myDevice;	// new
...
//	static ClientCallbacks *bmsCallback;
...
//	static NimBLEClient *pClient;
	...
	static int dataOk;					// number of valid bms data
	static int dataErr;					// bms data errors
	...
	void setBMS(bool state),
		 setCharge(bool state),
		 setDischarge(bool state),
		 setBalanceEnable(bool state),
		 setChargeBalance(bool state);
	byte getDebugLevel();
